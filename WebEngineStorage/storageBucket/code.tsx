...
export async function saveImage(bucketName:string,imageName:string,element:HTMLInputElement): Promise<SaveResponse> {
...
export async function loadImage(bucketName:string,imageName:string): Promise<SaveResponse> {
    if ('storageFoundation' in navigator) {
        const storageFoundation = (navigator as any).storageFoundation; // TypeScript workaround
        const bucket = await storageFoundation.open(bucketName);
...
export async function SaveImageWithBlob(bucketName:string,imageName:string,blob:Blob):Promise<SaveResponse>{
    if ('storageFoundation' in navigator) {
        const storageFoundation = (navigator as any).storageFoundation; // TypeScript workaround
        const bucket = await storageFoundation.open(bucketName);
